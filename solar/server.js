...
app.post('/upload', upload.single('file'), async (req, res) => {
  ...
  const selectedDate = req.body.date; // Get the selected date from the form
  ...
  try {
    ...
    for (let row of data) {
      const SN = row.SN || 'Unknown'; // Default to "Unknown" if empty
      const type = row.Type || 'Unknown'; // Default to "Unknown" if empty
      const siteName = row.SiteName || 'Unknown'; // Default to "Unknown" if empty
      const userName = row.UserName || 'Unknown'; // Default to "Unknown" if empty
      const ratedPower = row['Rated Power(kWh)'] || 0; // Default to 0 if empty
      const installer = row.Installer || 'Unknown'; // Default to "Unknown" if empty
      const inverterSN = row['Inverter SN'] || 'Unknown'; // Default to "Unknown" if empty
      const totalYield = row['Total Yield(kWh)'] || 0; // Default to 0 if empty
      const dailyYield = row['Daily Yield(kWh)'] || 0; // Default to 0 if empty
      const country = row.Country || 'Unknown'; // Default to "Unknown" if empty

      if (!SN) {
        console.log('Skipping row with missing Serial Number (SN):', row);
        continue; // Skip this row if SN is missing
      }
      ...
      if (!inverter) {
        ...
        // If the inverter exists, add the new daily yield
        inverter.dailyYields.push({ date: accessTime, yield: dailyYield });

        // Optionally update other fields if they can change over time
        inverter.type = type;
